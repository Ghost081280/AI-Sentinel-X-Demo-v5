 ...
 * Enhanced with internal-first discovery and Shodan API integration
 */
...
function startAutoScan() {
    console.log('Starting internal discovery scan...');
    ...
    // Enhanced scan steps for internal-first discovery
    const scanSteps = [
        { progress: 10, text: "Initializing internal discovery..." },
        { progress: 25, text: "Deploying network discovery agent..." },
        { progress: 40, text: "Scanning internal network topology..." },
        { progress: 55, text: "Enumerating devices and services..." },
        { progress: 70, text: "Analyzing network infrastructure..." },
        { progress: 85, text: "Determining optimal security configuration..." },
        { progress: 100, text: "Internal discovery complete! Configuring interface..." }
    ];
    ...
    if (window.sentinelChat && SentinelState.chatOpen) {
        sentinelChat.addMessage('🔍 NetworkMapper: Internal discovery initiated. Deploying agent to map network topology and identify endpoints. This will determine your optimal security package...', false, 'system');
    }
}

function simulateNetworkScan() {
    // Enhanced network configurations based on internal discovery
    const scenarios = [
        {
            type: 'individual',
            confidence: 94,
            details: {
                ...
                services: 6,
                complexity: 'Low',
                infrastructure: 'Single Server/Workstation',
...
function showScanResults(recommendation) {
    console.log('Showing internal discovery results:', recommendation);
    const scanProgress = document.getElementById('scanProgress');
    const autoScanBtn = document.getElementById('autoScanBtn');
    ...
    if (window.sentinelChat && SentinelState.chatOpen) {
        sentinelChat.addMessage(`🎯 NetworkMapper: Internal discovery complete!
Infrastructure Analysis: ${recommendation.details.internalDevices} devices discovered
Network Complexity: ${recommendation.details.complexity}
    ...
    modal.innerHTML = `
        <div class="modal scan-results-modal-content">
            <div class="modal-header">
                <h2 class="modal-title">🎯 Internal Network Discovery Complete</h2>
            </div>
            <div class="modal-content">
                <div class="scan-results-summary">
                    <div class="summary-grid">
                        ...
                        <div class="summary-item">
                            <span class="summary-label">Network Ranges</span>
                            <span class="summary-value">${recommendation.details.networkRanges} Discovered Ranges</span>
                        </div>
                        <div class="summary-item">
                        ...
                        <div class="summary-item">
                            <span class="summary-label">Security Assessment</span>
                    ...
                    <div class="recommendation-banner">
                        ...
                        <div class="banner-content">
                            ...
                            <p style="margin-top: 10px; font-size: 14px; opacity: 0.8;">
                                External reconnaissance via Shodan API can be added after setup for complete visibility.
                            </p>
                        </div>
                    </div>
                </div>
...
function proceedToCheckout(packageType) {
    ...
    // Apply configuration and show network interface
...
function showCheckoutSection(packageType) {
    ...
    if (window.sentinelChat) {
        sentinelChat.addMessage(`💳 Ready for checkout! ${plan.name} plan selected based on internal discovery. Total: $${totalMonthly.toFixed(2)}/month. External reconnaissance via Shodan API can be added after activation.`, false, 'system');
    }
}
...
function processCheckout() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            // Show Shodan API overlay for external scanning setup
            showShodanApiSetup();
...
// NEW: Show Shodan API setup after activation
function showShodanApiSetup() {
    ...
    if (overlay) {
        ...
        // Update chat
        if (window.sentinelChat) {
            sentinelChat.addMessage('🔍 NetworkMapper: Internal discovery active! Add your Shodan API key to enable external reconnaissance and complete network visibility.', false, 'system');
        }
    }
}
...
function selectScale(scale, userSelected = true) {
    ...
    // Show and populate scanning sections
    ...
    // Update chat context with internal-first discovery focus
    if (window.sentinelChat && SentinelState.chatOpen && userSelected) {
        sentinelChat.addMessage(`NetworkMapper: Configured for ${config.text.toLowerCase()} scale. Internal discovery active. External reconnaissance via Shodan API available for setup. Coordinating with ThreatScanner, EncryptionDeployer, and DefenseOrchestrator for comprehensive ${config.chatContext}.`, false, 'system');
    }
}
...
function configureInterfaceForScale(scale, config) {
    ...
    if (scanDetails) {
        scanDetails.textContent = 'Internal Discovery Active';
    }

    // Update dashboard description
    const descriptions = {
        individual: '🤖 <strong>Server monitoring active</strong> • Internal discovery complete • External scanning optional',
        business: '🤖 <strong>Multi-site internal scanning</strong> • Network topology mapped • Shodan API available',
        enterprise: '🤖 <strong>Enterprise internal scanning</strong> • Global infrastructure mapped • External reconnaissance enabled'
    };
...
function populateScanningPanels() {
   // Check if Shodan API is connected
   const shodanApiKey = localStorage.getItem('shodan_api_key');
   shodanApiConnected = !!shodanApiKey;
   
   // Internal services data (always available)
   ...
   // Populate internal services (always available)
...
function addIPRange() {
   ...
   if (SentinelState.chatOpen) {
       setTimeout(() => {
           sentinelChat.addMessage(`NetworkMapper: New ${currentScale} range ${name} (${range}) added to monitoring. Initiating internal discovery scan across ${newRange.devices} estimated devices...`, false);
       }, 1000);
   }
}
...
function confirmRescan() {
   ...
   setTimeout(() => {
       sentinelChat.addMessage('NetworkMapper: Re-initializing internal discovery engine...', false, 'system');
   }, 2000);
...
function resetToInitialState() {
   ...
   localStorage.removeItem('shodan_api_key'); // Also clear Shodan API key
   ...
   if (detectionDescription) {
       detectionDescription.textContent = 'AI Sentinel-X has reset the network discovery configuration. Please run the internal discovery scan to re-analyze your infrastructure and configure the optimal security solution.';
   }
   
   // Add final chat message
   setTimeout(() => {
       sentinelChat.addMessage('✅ Network discovery reset complete. Environment detection reinitialized. Please run the internal discovery scan to continue.', false, 'system');
   }, 500);
}
...
function showOverviewDetails(type) {
   ...
   setTimeout(() => {
       const messages = {
           serverIP: 'NetworkMapper: Server IP analysis - Single address discovered via internal scan. External reconnaissance available via Shodan API.',
           ...
           businessSites: 'NetworkMapper: Multi-site business deployment with internal discovery. External reconnaissance available via Shodan API.',
           enterpriseRanges: 'NetworkMapper: Enterprise multi-range deployment across data centers. Full correlation with ThreatScanner.',
           devices: 'NetworkMapper: Device inventory analysis - All endpoints monitored by internal agent. EncryptionDeployer verified.',
...
// NEW: Shodan API Functions
window.setupShodanApi = setupShodanApi;
window.useDemoShodan = useDemoShodan;
window.skipShodanSetup = skipShodanSetup;
window.showShodanApiSetup = showShodanApiSetup;
window.populateExternalServices = populateExternalServices;
